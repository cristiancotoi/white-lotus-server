...
function BaZiCalculator(person) {
    ...
    function compute() {
        ...
        let astroData;
        let date = person;
        if (!_.isObject(person.date)) {
            date = getExpandedDate(person);
        }
        astroData = astroCalc.getData(date);
        ...
        if(!_.isUndefined(astroData.gender)) {
        ...
        // Verify that a person is being analyzed, not just a date
        if(!_.isUndefined(forward)) {
